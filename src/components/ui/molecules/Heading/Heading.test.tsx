...
describe('<Heading />', () => {
  it('Deve renderizar todos os componentes o componente <Heading />', () => {
    ...
    expect(
      screen.getByRole('heading', { name: /Bem Vindo a BarberShop/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('heading', { name: /We Are BarberShop!/i })
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        /Enjoy the Ultimate Relaxed Grooming Experience For Men every time you step into the Boardroom/i
      )
    ).toBeInTheDocument();

    expect(
      screen.getByRole('button', { name: /Marcar Hora/i })
    ).toBeInTheDocument();
    expect(
  ...
  it('Deve renderizar componente <Heading /> e verificar se os botões não são renderizados', () => {
    ...
    expect(
      screen.queryByRole('button', { name: /Marcar Hora/i })
    ).not.toBeInTheDocument();
  });

  it('Deve renderizar componente <Heading /> e verificar se só um botão é renderizados', () => {
    renderWithTheme(
      <Heading buttonLabelOne="Marcar Hora" {...props}></Heading>
    );
  ...
  it('Deve renderizar o conteúdo do componente <Heading /> e verificar a margem dos botões', () => {
    ...
    expect(
      screen.getByRole('button', { name: /Marcar Hora/i }).parentElement
    ).toHaveStyle({
