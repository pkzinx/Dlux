...
describe('<SectionContact />', () => {
  ...
  it('Deve renderizar o componente <SectionContact />', () => {
    ...
    const selectRecommend = screen.getByRole('combobox', {
      name: 'Recomendaria este serviço?',
    });
  ...
  it('Deve verificar quando o campo `Atendido por` for vazio no componente <SectionContact />', () => {
    //Arrange
    const selectClerk = screen.getByRole('combobox', {
      name: 'Atendido por',
    }) as HTMLInputElement;
    ...
    //Assert
    expect(photoClerk).toHaveStyle({
      backgroundImage: `url('/assets/img/foto_indisponivel.jpg')`,
    });
  });

  it('Deve verificar quando o campo `Atendido por` for selecionado no componente <SectionContact />', async () => {
    //Arrange
    const selectValueClerk = screen.getByRole('option', {
      name: /Cleber Mariano/i,
    ...
    expect(photoClerk).toHaveStyle({
      backgroundImage: `url('/assets/img/contributors-1.jpg')`,
    });
  });
